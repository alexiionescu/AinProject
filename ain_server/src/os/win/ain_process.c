...
ain_process_t *ain_process_fork() {
  ...
  if (!memcmp(sysinfo->argv[0], "ain_child_process",
              sizeof("ain_child_process")))
    return NULL;
  ...
  AIN_ASSERT_NULL(CreateProcess(sysinfo->argv[0], "ain_child_process", NULL,
...
int ain_process_is_alive(ain_process_t *p) {
  if (WaitForSingleObject(p->process, 0) == WAIT_TIMEOUT) return 1;
