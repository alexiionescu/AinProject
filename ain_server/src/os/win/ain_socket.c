...
ain_result_t ain_sockets_init() {
  ...
  DWORD bytes;
  AIN_ASSERT_ERR(0 == WSAIoctl(s, SIO_GET_EXTENSION_FUNCTION_POINTER, &ax_guid,
                               sizeof(GUID), &ain_acceptex,
                               sizeof(LPFN_ACCEPTEX), &bytes, NULL, NULL));
  AIN_ASSERT_ERR(0 == WSAIoctl(s, SIO_GET_EXTENSION_FUNCTION_POINTER, &as_guid,
                               sizeof(GUID), &ain_getacceptexsockaddrs,
                               sizeof(LPFN_GETACCEPTEXSOCKADDRS), &bytes, NULL,
                               NULL));
  AIN_ASSERT_ERR(0 == WSAIoctl(s, SIO_GET_EXTENSION_FUNCTION_POINTER, &tf_guid,
                               sizeof(GUID), &ain_transmitfile,
                               sizeof(LPFN_TRANSMITFILE), &bytes, NULL, NULL));
  AIN_ASSERT_ERR(0 == WSAIoctl(s, SIO_GET_EXTENSION_FUNCTION_POINTER, &tp_guid,
                               sizeof(GUID), &ain_transmitpackets,
                               sizeof(LPFN_TRANSMITPACKETS), &bytes, NULL,
                               NULL));
  AIN_ASSERT_ERR(0 == WSAIoctl(s, SIO_GET_EXTENSION_FUNCTION_POINTER, &cx_guid,
                               sizeof(GUID), &ain_connectex,
                               sizeof(LPFN_CONNECTEX), &bytes, NULL, NULL));
  AIN_ASSERT_ERR(0 == WSAIoctl(s, SIO_GET_EXTENSION_FUNCTION_POINTER, &dx_guid,
                               sizeof(GUID), &ain_disconnectex,
                               sizeof(LPFN_DISCONNECTEX), &bytes, NULL, NULL));
  return AIN_OK;
}
...
ain_result_t ain_socket_accept(ain_event_t *evt) {
  ...
  if (ain_acceptex(conn->ls->fd, conn->fd, EVENT_BUFFER(evt), 0, addrSize,
                   addrSize, &dwReceivedBytes, (LPOVERLAPPED)evt)) {
    evt->handler(evt);
    return AIN_OK;
  }
...
ain_result_t ain_socket_onaccept(ain_event_t *evt) {
  ...
  ain_getacceptexsockaddrs(buf, 0, addrSize, addrSize, &lsaddr, &lsaddrlen,
                           &rsaddr, &rsaddrlen);
  conn->connected = 1;
...
ain_result_t ain_socket_read(ain_event_t *evt) {
  ...
  if (WSA_IO_PENDING == WSAGetLastError()) return AIN_OK;
...
ain_result_t ain_socket_write(ain_event_t *evt) {
  ...
  memset(evt, 0, sizeof(OVERLAPPED));
  if (0 == WSASend(evt->fd, &DataBuf, 1, &dwSentBytes, Flags, (LPOVERLAPPED)evt,
                   NULL)) {
    evt->error = 0;
