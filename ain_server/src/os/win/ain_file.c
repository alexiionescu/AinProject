...
ain_result_t ain_file_write(ain_event_t* evt)
{
	...
	BOOL bRet = WriteFile((HANDLE)evt->fd, EVENT_BUFFER(evt), evt->buf_size,
		NULL, (LPOVERLAPPED)evt);
	if (bRet)
	{
...
{
	memset(evt, 0, sizeof(OVERLAPPED));
	BOOL bRet = ReadFile((HANDLE)evt->fd, EVENT_BUFFER(evt), EVENT_BUFFER_SIZE(evt),
		NULL, (LPOVERLAPPED)evt);
	if (bRet)
	{
		evt->buf_size = (uint32_t)evt->iocp_id.InternalHigh;
...
void ain_file_close(ain_fd_t fd)
{
