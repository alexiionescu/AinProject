...
#ifdef _MSC_VER
#define MMAP_ALLOC(size) \
  VirtualAlloc(NULL, size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE)

#define MMAP_FREE(ptr, size) VirtualFree(ptr, 0, MEM_RELEASE)
#elif __GNUC__
#include <sys/mman.h>
#define MMAP_ALLOC(size) \
  mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, 0, 0)

#define MMAP_FREE(ptr, size) munmap(ptr, size)
#endif
...
ain_mem_pool_t *ain_pool_create(uint16_t pool_size, uint32_t max_pools) {
  if (pool_size < AIN_MIN_POOL_SIZE) pool_size = AIN_MIN_POOL_SIZE;
  if (max_pools == 0) max_pools = 1;  // at least 1 pool
  size_t req =
      (sizeof(ain_mem_pool_t) + max_pools * SIZEOF_MEMORY_POOL(pool_size));
  AIN_BYTES_TO_PAGES(req);

  ain_mem_pool_t *_pool = MMAP_ALLOC(pool_size * AIN_PAGE_SIZE);
  ...
  for (size_t k = 0; k < req; k++) pool->pages[k] = (uint8_t)(1 + k);
  return _pool;
}
...
void *ain_pool_alloc(ain_mem_pool_t *_pool, size_t req) {
  ...
  ain_memory_pool_t *pool = GET_FIRST_POOL(_pool);
  while (j - i < req) {
    while (i < _pool->pool_size && pool->pages[i]) i++;
    if (i < _pool->pool_size) {
      j = i + 1;
      while (j < _pool->pool_size && !pool->pages[j] && j - i < req) j++;
    }
    if (i == _pool->pool_size || (j == _pool->pool_size && j - i < req)) {
      ...
      if (!npool->base) {
        ...
        npool->base = MMAP_ALLOC(_pool->pool_size * AIN_PAGE_SIZE);
  ...
  for (size_t k = i; k < j; k++) pool->pages[k] = (uint8_t)(1 + k - i);
...
void ain_pool_free(ain_mem_pool_t *_pool, void *p) {
  ...
  while (k < (int32_t)_pool->pool_size - 1 &&
         pool->pages[k] < pool->pages[k + 1])
    pool->pages[k++] = 0;
  if (k < (int32_t)_pool->pool_size) pool->pages[k] = 0;
}
...
void ain_pool_destroy(ain_mem_pool_t *_pool) {
  ...
  for (uint32_t i = 1; i < _pool->pool_cnt; i++) {
    pool = GET_NEXT_POOL(pool, _pool);
    MMAP_FREE(pool->base, _pool->pool_size * AIN_PAGE_SIZE);
  }
  MMAP_FREE(_pool, _pool->pool_size * AIN_PAGE_SIZE);
}
