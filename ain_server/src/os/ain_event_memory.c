...
ain_event_t *ain_event_alloc(ain_mem_pool_t *pool, size_t data_size, size_t pages, uint32_t	blocks)
{
    ain_event_t *evt = (ain_event_t *)malloc(sizeof(ain_event_t) + data_size + sizeof(struct iovec) * blocks);
    AIN_ASSERT_NULL(evt);
    evt->iovcnt = blocks;
