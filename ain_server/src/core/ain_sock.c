...
ain_listeners_t *ain_listeners_conf(ain_conf_t *conf) {
  ...
  for (size_t l = 0; l < lsdata->lls_size; l++) {
    ...
    if (AIN_OK != ain_proto_parse_uri(&ls->uri, &buf, &size)) continue;
    ...
    if (AIN_OK !=
        ain_inet_pton(ls->sockaddr.sa_family, ls->uri.scheme + ls->uri.host,
                      ls->uri.host_size, &ls->sockaddr, &ls->sockaddrlen))
      continue;

    ain_inet_setport(
        ls->sockaddr.sa_family,
        ain_proto_get_default_port(ls->uri.scheme, ls->uri.scheme_size),
        ls->uri.scheme + ls->uri.port, ls->uri.port_size, &ls->sockaddr,
        ls->sockaddrlen);
  }
  return lsdata;
}
