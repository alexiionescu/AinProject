...
ain_result_t ain_listen_event(ain_event_t *evt) { return AIN_OK; }
...
ain_result_t ain_worker_proc(void *data) {
    ...
    if (IO_QUERY_CNT != ior) {
      ...
      if (w_conns) {
        ...
        for (size_t l = 0; l < ls_size; l++) {
          ...
          for (size_t c = 0; c < ls->conn_size; c++) {
            if (conn->fd == INVALID_SOCKET && wdata->time - conn->time > 0)
              ain_conn_accept(conn);  // re-use closed socket
            else if (conn->keep_alive && conn->connected &&
                     wdata->time - conn->time > 10)
  ...
  } while (wdata->quit != AIN_WORKER_QUIT_REQ);
