...
ain_result_t ain_child_process()
{
	...
	int shm_id;
	worker_data_t* workers_data = (worker_data_t*)ain_shared_data_alloc(conf.w_max ,sizeof(worker_data_t),&shm_id);
	...
	do
	{
		...
		for (uint32_t i = 0; i < conf.w_max; i++)
		{
			switch (workers_data[i].quit)
			{
				...
				default:
					if(_sig_stopped)
						workers_data[i].quit = AIN_WORKER_QUIT_REQ;
			}
		}
	}while (join);
...
ain_result_t ain_parent_process_ux()
{
	...
	worker_data_t* workers_data = (worker_data_t*)ain_shared_data_alloc(conf.w_max ,sizeof(worker_data_t),&shm_id);
	AIN_ASSERT_ERR(workers_data);
	ain_process_t** workers = (ain_process_t**)malloc(conf.w_max * sizeof(ain_process_t*));
	for (uint8_t i = 0; i < conf.w_max; i++)
	{
