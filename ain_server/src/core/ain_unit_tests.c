...
uint8_t _uri_tests[] = {
	...
	"   https://username:password@doamin.com:2021/a/b/test/index.html?q1=232&q2=343#sub-chapter\r"
			...
			while (AIN_OK == ain_proto_parse_uri(&uri,&buf, &s))
			{
				if (i == 0 && t == 0)
				{
					printf("<%.*s>://<%.*s>@<%.*s>:<%.*s> <%.*s>?<%.*s>#<%.*s>\n",
						(int)uri.scheme_size, uri.scheme,
						(int)uri.userinfo_size, uri.scheme + uri.userinfo,
						(int)uri.host_size, uri.scheme + uri.host,
						(int)uri.port_size, uri.scheme + uri.port,
						(int)uri.path_size, uri.scheme + uri.path,
						(int)uri.query_size, uri.scheme + uri.query,
						(int)uri.fragment_size, uri.scheme + uri.fragment
					);
				}
			}
