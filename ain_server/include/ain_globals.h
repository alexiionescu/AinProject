...
#define AIN_ASSERT_CONTINUE \
  PUSH_WARNINGS()           \
  ain_result_t *__p = NULL; \
  *__p = 0 POP_WARNINGS()
#endif

#define AIN_ASSERT_START(cond)                                        \
  do {                                                                \
    if (!(cond)) {                                                    \
      ...
      fprintf(stderr, "%s:%d error %d %s\n", __FILE__, __LINE__, err, \
              strerror(err));                                         \
      AIN_ASSERT_CONTINUE;
#define AIN_ASSERT_END \
  }                    \
  }                    \
  while (0)

#define AIN_ASSERT(cond)         \
  AIN_ASSERT_START(cond) return; \
  AIN_ASSERT_END
#define AIN_ASSERT_NULL(cond)         \
  AIN_ASSERT_START(cond) return NULL; \
  AIN_ASSERT_END
#define AIN_ASSERT_ERR(cond)               \
  AIN_ASSERT_START(cond) return AIN_ERROR; \
  AIN_ASSERT_END
...
#define AIN_EQUALS(b, s, str) (s == sizeof(str) - 1 && 0 == memcmp(b, str, s))
